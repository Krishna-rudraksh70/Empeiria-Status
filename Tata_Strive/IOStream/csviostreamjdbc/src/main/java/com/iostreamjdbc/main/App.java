...
import com.iostreamjdbc.config.AppConfig;
import com.iostreamjdbc.config.HibernateConfig;
import com.iostreamjdbc.model.Author;
import com.iostreamjdbc.model.Book;
import com.iostreamjdbc.service.AuthorService;

import com.iostreamjdbc.service.BookService;
...
public class App {
    ...
    private static FileWriter fileWriterTxt = null;
    private static BufferedWriter bufferedWriterTxt = null;
    private static FileWriter fileWriterCsv = null;
    private static BufferedWriter bufferedWriterCsv = null;

    public static void initializeBufferedWriter() {
        try {
            fileWriterTxt = new FileWriter("src/main/java/com/iostreamjdbc/database/libraryData.txt", true);
            bufferedWriterTxt = new BufferedWriter(fileWriterTxt);
            fileWriterCsv = new FileWriter("src/main/java/com/iostreamjdbc/database/libraryData.Csv", true);
    ...
    public static void closeBufferedWriter() {
        try {
            if (bufferedWriterTxt != null || bufferedWriterCsv != null) {
    ...
    public static void writeToTxt(Book book) throws IOException {
        bufferedWriterTxt.write(++entry + " -> " + book);
        bufferedWriterTxt.newLine();
        bufferedWriterTxt.flush();
    }

    public static void writeToCsv(Book book) throws IOException {
        bufferedWriterCsv.write(String.format("%d, %d, %s, %d, %d, %d, %s",
                ++entry, book.getBId(), book.getBTitle(),
                book.getPublicationYear(), book.getCopies(),
                book.getBAuthor().getAId(), book.getBAuthor().getAName()));
    ...
    private static void getAuthors() {
        ...
        authors.forEach(author -> {
            System.out.println(author.getAId() + ". " + author.getAName());
        });
    }
    ...
    private static void getBooks() {
        ...
        System.out.println("\nAll Books:");
        books.forEach(book -> {
            System.out.println(book.getBId() + ". " + book.getBTitle());
            try {
                writeToTxt(book);
                writeToCsv(book);
            } catch (IOException e) {
